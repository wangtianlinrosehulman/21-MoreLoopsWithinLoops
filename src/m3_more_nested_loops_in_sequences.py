...
def largest_number(seq_seq):
    ...
    # ------------------------------------------------------------------
    large = -1000000000000000000000000000000000000000000000000
    result = large
    for k in range(len(seq_seq)):
        for i in range(len(seq_seq[k])):
            if seq_seq[k][i] > large:
                large = seq_seq[k][i]
            if large == result:
                return None
    return large
...
def largest_negative_number(seq_seq):
    ...
    # ------------------------------------------------------------------
    large = -100000000000000000
    result = large
    for k in range(len(seq_seq)):
        for i in range(len(seq_seq[k])):
            if seq_seq[k][i] < 0:
                if abs(seq_seq[k][i]) < abs(large):
                    large = seq_seq[k][i]
    if large == result:
        return None
    return large
...
def first_is_elsewhere_too(seq_seq):
    ...
    for k in range(1, len(seq_seq)):
        for i in range(len(seq_seq[k])):
            for a in range(len(seq_seq[0])):
                if seq_seq[0][a] == seq_seq[k][i]:
                    return True
    return False
